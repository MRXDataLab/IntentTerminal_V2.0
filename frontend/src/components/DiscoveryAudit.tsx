...
import { Database, ShieldAlert, BarChart3, Radio, ArrowLeft, Upload } from 'lucide-react';

interface DiscoveryAuditProps {
  manifest: Record<string, any>;
  onBeginIngestion: () => void;
  onBack: () => void;
}

export default function DiscoveryAudit({ manifest, onBeginIngestion, onBack }: DiscoveryAuditProps) {
  ...
  useEffect(() => {
    const fetchSources = async () => {
      try {
        // Extract discovery targets from the manifest
        const discoveryNodes: string[] = [];

        // Pull from boolean_nets target_signals
        if (manifest.boolean_nets) {
          for (const net of manifest.boolean_nets) {
            if (net.target_signals) {
              discoveryNodes.push(...net.target_signals);
            }
          }
        }

        // Pull from entity_anchors
        if (manifest.entity_anchors) {
          if (manifest.entity_anchors.primary_brand) {
            discoveryNodes.push(manifest.entity_anchors.primary_brand);
          }
          if (manifest.entity_anchors.tracked_competitors) {
            discoveryNodes.push(...manifest.entity_anchors.tracked_competitors);
          }
        }

        // Deduplicate
        const uniqueNodes = [...new Set(discoveryNodes)];

        const res = await axios.post('http://localhost:8000/api/discover', {
          graph_nodes: uniqueNodes.length > 0 ? uniqueNodes : ['General Research Topic'],
        });
  ...
  if (loading) {
    return (
      <div className="flex w-full h-screen bg-black text-white items-center justify-center flex-col relative overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(245,158,11,0.08)_0,transparent_50%)]"></div>
        <Radio size={48} className="text-amber-500 animate-pulse mb-6" />
        <h2 className="text-2xl font-light tracking-widest text-amber-50">INITIATING LINK-FARM CRAWL</h2>
        <p className="text-gray-500 mt-2">Scouting and dipping across sources to measure signal...</p>
        <div className="mt-6 bg-[#111] border border-[#222] rounded-xl p-4 max-w-md w-full">
          <div className="text-xs text-gray-500 font-mono uppercase tracking-widest mb-2">Manifest Summary</div>
          <div className="flex gap-4 text-xs text-gray-400">
            <span>{manifest.boolean_nets?.length || 0} boolean nets</span>
            <span>{manifest.signal_taxonomy?.length || 0} signal tags</span>
            <span>{manifest.entity_anchors?.tracked_competitors?.length || 0} rivals</span>
          </div>
        </div>
      </div>
    );
    ...
    <div className="flex flex-col w-full h-screen bg-[#050505] text-white p-8 overflow-y-auto">
      <div className="max-w-7xl mx-auto w-full space-y-8">
        ...
        <div className="flex items-center justify-between border-b border-[#333] pb-6">
          <div className="flex items-start gap-4">
            ...
            <div className="flex flex-col">
              ...
              <h1 className="text-3xl font-medium tracking-tight">Discovery & Audit</h1>
              <p className="text-sm text-gray-500 mt-2">Module 3 — Manifest-Driven Source Triage & Scale Estimate</p>
            </div>
          </div>
          <button
            onClick={onBeginIngestion}
            className="px-6 py-3 bg-amber-500 hover:bg-amber-400 text-black font-semibold rounded-lg shadow-[0_0_20px_rgba(245,158,11,0.3)] transition-all flex items-center gap-2"
          >
            <Database size={18} />
        ...
        <div className="grid grid-cols-4 gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-amber-950/20 p-6 rounded-2xl border border-amber-900/30">
            <h3 className="text-amber-400 text-sm font-medium mb-1">High-Signal Containers</h3>
            <div className="text-4xl font-light text-amber-50">{data?.approved_sources}</div>
            <div className="text-xs text-amber-600 mt-2">Passed Neutrality & Relevance test</div>
          </motion.div>
              ...
              <tbody className="divide-y divide-[#222]">
                {data?.sources?.map((src: any, index: number) => (
                  <tr key={index} className="hover:bg-[#151515] transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 rounded-md text-xs font-semibold ${src.signal_score >= 0.7 ? 'bg-amber-500/10 text-amber-400' : 'bg-yellow-500/10 text-yellow-400'}`}>
                        {src.signal_score}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      {src.status === 'APPROVED' ? (
                        <div className="flex items-center gap-1.5 text-amber-500 text-xs font-medium uppercase">
                          <div className="w-1.5 h-1.5 rounded-full bg-amber-500"></div> Approved
                        </div>
                      ) : (
